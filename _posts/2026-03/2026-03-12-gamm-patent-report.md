...
**Abstract**:  
一种示例方法包括在第一模式下显示多个文本词。在显示多个文本词的第一模式期间，检测用户执行的第一个空中手势。响应于第一个空中手势，启用与第一模式不同的文本修改模式，允许用户识别并修改多个文本词中的至少一个。基于至少部分用户的第一动作识别目标词，并在文本修改模式启用时，基于至少部分用户的第二动作修改目标词。该方法进一步包括，在第一模式下，无法识别或修改任何显示的文本词。",
